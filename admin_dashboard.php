    ...
    <div class="product-page product-pop-up">
      <div class="row">
        ...
        <div class="col-md-6 col-sm-6 col-xs-9">
          <h2 id="title_job">เพิ่มรายกาารโฆษณาหลักใหม่</h2>
        ...
        return {
          ...
          onClickBtnEditMainAdvertising: function () {
            $('body').on('click', '#cs_table_main_advertising_wrapper [data-btn="edit"]', function () {
              ...
              var key_word = $(this).data('keyword');
              var order = $(this).data('order');
              var tol = $(this).data('tol');
              var email = $(this).data('email');
              Maha.requestFileListData(advertising_id);
          ...
          requestFileListData: function (id) {
            var data = new FormData();
            data.append('job_id', id);
            $.ajax({
              url: "admin_db_webrequest_files_job.php",
              method: "POST",
              data: data,
              processData: false,  // อย่าจัดการข้อมูลเอง
              contentType: false,  // ประเภทข้อมูลอ้างอิงไปที่ไฟล์แนบ
              success: function (response) {
                ...
                console.log(result);
                Maha.initFileEditDropzone(result);
              }
            });
          },
          initFileEditDropzone: function (data) {
            var dz_img = '';
            data.forEach(function (e) {
              dz_img += `<div class="dz-preview dz-image-preview dz-complete">
                            <div class="dz-image dz-file-type-1">
                                <img data-dz-thumbnail="" alt="" onerror="this.style.display = 'none'" style="width:100%;" src="upload/img_jobs_approved/${e.job_id}/${e.file_name}" />
                            </div>
                            <div class="dz-details">
                                <div class="dz-filename"><span data-dz-name="">${e.file_name}</span></div>
                            </div>
                            <div class="dz-progress"><span class="dz-upload" data-dz-uploadprogress=""></span></div>
                            <div class="dz-error-message"><span data-dz-errormessage=""></span></div>
                            <div class="dz-success-mark"></div>
                            <div class="dz-error-mark"></div>
                            <a class="dz-remove" onclick="Feature.removeFilesFromDropzone(this)" data-id="${e.id}" data-fileName="${e.file_name}" data-dz-remove="">ลบ</a>
                        </div>`
            });

            $('#dz_upload').html(dz_img);
          },
          submitNewAdvertising: function (active) {
            ...
            if (active == 1) {
              ...
              data.append('key_word', ct_keyword);
