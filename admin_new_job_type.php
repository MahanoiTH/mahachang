...
<body class="ecommerce">
            ...
            <div class="row margin-bottom-40">
                ...
                <div class="col-sm-7">
                    <!-- <select class="form-control input-lg" id="select_jobs">
                        <option value="1">รายการโฆษณาหลัก</option>
                        <option value="2">งานออกแบบ ตกแต่งภายใน</option>
                        <option value="3">งานระบบแอร์ ระบบปรับอากาศ</option>
                        <option value="4">งานระบบไฟฟ้า งานระบบไฟฟ้า</option>
                        <option value="5">งานระบบประปา งานระบบประปา</option>
                        <option value="6">งานเหล็ก งานโครงสร้างเหล็ก</option>
                        <option value="7">งานปูพื้น ปูพื้นกระเบื้อง</option>
                        <option value="8">งานเขียนแบบ ออกแบบบ้าน</option>
                        <option value="9">งานสร้างบ้าน และอสังหาฯอื่นๆ</option>
                    </select> -->
                </div>
                <div class="col-sm-5">
            ...
            <div class="row">
                <div class="col-md-12 col-sm-12 col-xs-12">
                    ...
                    <div class="price-availability-block clearfix">
                        <div class="cs-name">
                            <label for=""><h4>ชื่องาน</h4></label><br />
                            <textarea id="ct_name" name="ct_name" rows="4" cols="90"></textarea>
                        </div>
                        <div class="cs-name">
    ...
    <script type="text/javascript">
        ...
        var attc_list = [];
        var attc_list_name = [];
        var data_url = 'admin_db_datatable_job_type.php';
        var submit_jobs_url = "admin_db_submit_job_type.php";
                ...
                return {
                    ...
                    dataTableListAdvertising: function () {
                        ...
                        dataTable = $('#cs_table_main_advertising').DataTable({
                            ...
                            columnDefs: [
                                { width: '5%', targets: 0 }, // กำหนดความกว้างของคอลัมน์ที่ 0
                                { width: '30%', targets: 1 }, // กำหนดความกว้างของคอลัมน์ที่ 1
                                { width: '55%', targets: 2 }, // กำหนดความกว้างของคอลัมน์ที่ 2
                                { width: '10%', targets: 3 }, // กำหนดความกว้างของคอลัมน์ที่ 2
                            ],
                            columns: [
                                { data: 'id'},
                                { data: 'name' },
                                { data: 'keyword' },
                                {
                                    data: function (data) {
                                        ...
                                        return `<button data-btn="edit" data-id="${data.id}" data-name="${data.name}" data-keywords="${data.keyword}" data-folder_name="${data.folder_name}" href="#modal_add_new_advertising" type="button" class="fancybox-fast-view btn btn-default" ><i class="fa fa-edit"></i></button><button data-id="${data.id}" data-btn="delete" type="button" class="btn btn-danger" ><i class="fa fa-trash-o"></i> </button>`;
                                    },
                                }
                            ],
                            ajax: {
                    ...
                    onClickBtnSaveAddNewAdvertising: function () {
                        $('#btn_save_new_advertising').on('click', function () {
                            ...
                            ct_name = $('#ct_name').val();
                            ct_folder_name = $('#ct_folder_name').val();
                            ct_keywords = $('#ct_keywords').val();
                    ...
                    onClickBtnEditMainAdvertising: function () {
                        $('body').on('click', '#cs_table_main_advertising_wrapper [data-btn="edit"]', function () {
                            ...
                            $('#ct_tol').val(tol);
                            $('#ct_email').val(email);
                            $('#ct_folder_name').val(folder);
                            $('#ct_keywords').val(keywords);
                    ...
                    submitNewAdvertising: function (active) {
                        ...
                        if (active == 1) {
                            ...
                            data.append('ct_keywords', ct_keywords);
                            data.append('ct_folder_name', ct_folder_name);
