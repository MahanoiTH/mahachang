...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id == 0) {
        ...
        if (empty($ct_name) || empty($ct_desc) || empty($ct_order) || empty($start_date) || empty($end_date) || empty($active)) {
            ...
            $sql = "INSERT INTO jobs_air_conditioning (job_order, active, created_by, start_date, end_date, client_name, job_description, phone_number, email) 
        ...
        if (empty($active) && empty($id)) {
            ...
            if ($active == 0) {
                ...
                $sql = "UPDATE jobs_air_conditioning 
                        SET active = '$active'
                        WHERE id = $id";
            } else {
                $ct_name = $_POST['ct_name'];
